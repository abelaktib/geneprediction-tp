...
def find_stop(stop_regex, sequence, start):
    """Find the stop codon
    """
    result = list(stop_regex.finditer(sequence, start))
    for i in result:
        if i != None:
...
def has_shine_dalgarno(shine_regex, sequence, start, max_shine_dalgarno_distance):
    ...
    """
    end = start - max_shine_dalgarno_distance
    debut = start - 6
    result = shine_regex.search(sequence)

    if result is not None:
        if debut >= result.start(0) + 1 >= end:
            if debut >= result.end(0) + 1 >= end:
                return True
    return False
...
def predict_genes(sequence, start_regex, stop_regex, shine_regex,
    ...
    while len(sequence) - position_courante >= min_gap:
        position_courante = find_start(start_regex, sequence, position_courante,
                                       len(sequence))
        ...
        if position_courante is not None:
            ...
            if stop is not None:
                ...
                long = stop - position_courante

                if long >= min_gene_len:

                    if has_shine_dalgarno(shine_regex, sequence, position_courante, max_shine_dalgarno_distance):
                        gene.append([position_courante + 1, stop + 3])
                        position_courante = position_courante + stop + 2 + min_gap
                    else:
                        position_courante += 1
    ...
    try:
        with open(fasta_file, "wt") as fasta:
            ...
            for j, gene_pos in enumerate(probable_genes_comp):
                fasta.write(">gene_{0}{1}{2}{1}".format(i+1+j, os.linesep,
...
def main():
    ...
    # Arguments
    args = get_arguments()
    seq1 = read_fasta(args.genome_file)
    seq2 = reverse_complement(seq1)
    predseq1 = predict_genes(seq1, start_regex, stop_regex, shine_regex,
                             args.min_gene_len, args.max_shine_dalgarno_distance, args.min_gap)
    predseq2 = predict_genes(seq2, start_regex, stop_regex, shine_regex,
                             args.min_gene_len, args.max_shine_dalgarno_distance, args.min_gap)

    write_genes_pos("position.csv", predseq1)

    write_genes("prodigal.csv", seq1, predseq1, seq2, predseq2)
